...
def get_Cells(ell_max, bigell_max, z_cluster, cosmo_params, make_plots = False):
    ...
    #Update to included polarization
    ...
    return ell, clTT_unlensed, bigell, cl_kappa_highz, cl_kappa_lowz
...
def prepare_analysis(z_cluster, obs_type = 'spt3g_nobeam', prep_likelihood = False, make_plots = False, N_pix = 16, pix_size_arcmin = 0.5):
    ...
    ell_max = 3000
    bigell_max = 2000
    ell, clTT_unlensed, bigell, clpp_highz, clpp_lowz = get_Cells(ell_max, bigell_max, z_cluster, cosmo_params)
    ...
    if (prep_likelihood):
        ...
        cov_element_table = np.zeros(num_table)
        for ii in range(0, num_table):
            #note that we use unlensed CMB power spectrum here 
            cov_element_table[ii] = likelihood.get_cov_element(ell, clTT_unlensed, ang_sep_table[ii])
        cov_interp_func = interpolate.interp1d(ang_sep_table, cov_element_table)
        likelihood_info = {'cov_interp_func':cov_interp_func}
